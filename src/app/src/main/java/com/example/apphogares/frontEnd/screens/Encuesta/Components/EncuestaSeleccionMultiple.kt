...
@Composable
fun  EncuestaSeleccionMultiple(
    onDismiss : () -> Unit = { },
    onConfirmButton: (String) -> Unit = { },
    encuesta: Encuesta
){

    SeleccionMultipleScreen(
...
@Composable
fun SeleccionMultipleScreen(
    onConfirmButton: (String) -> Unit = { },
    encuesta: Encuesta,
    encuestaRespuestaViewModel: encuestaRespuestaViewModel = hiltViewModel()
){
//    var respuestaSeleccionada by remember { mutableStateOf("") }
//    val listaRespuestasSeleccionadas = remember { mutableStateListOf<String>() }
    val respuestas = encuesta.respuestas.split(";")
    Column(
        modifier = Modifier
            .fillMaxSize()
            ...
            respuestas.forEach { opcion ->
                item {
                    Row(
                        modifier = Modifier
                            .clickable() {
                                if (encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value.contains(opcion))
                                    encuestaRespuestaViewModel.removeItem(opcion)
                                else
                                    encuestaRespuestaViewModel.addItem(opcion)
                            }
                            .fillMaxWidth()
                            .background(
                                BackGroundTopLogin,
                        ...
                        Checkbox(
                            checked = encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value.contains(opcion),
                            onCheckedChange = {
                                println("listaRespuestasSeleccionadas $encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value")
                            ...
                            colors = CheckboxDefaults.colors(
                                checkedColor = BackGroundTopRuta,
                                uncheckedColor = Gray,
                                disabledCheckedColor = Gray,
                                disabledUncheckedColor = Gray
                            )
                        )
                        Text(text = opcion, style = MaterialTheme.typography.titleMedium)
                    }
        ...
        Row(modifier = Modifier
            ...
            .align(Alignment.CenterHorizontally)) {
            Button(
                enabled = encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value.isNotEmpty(),
                modifier = Modifier
                    .width(200.dp)
                    .background(
                ...
                onClick = {
                    if (encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value.isNotEmpty()){
                        encuestaRespuestaViewModel.listaRespuestasSeleccionadas.value.forEach {
                            encuestaRespuestaViewModel.respuestaSeleccionada.value += "$it,"
                        }
